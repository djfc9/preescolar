	...
	public function view($id) {
		/*if (!$this->Persona->exists($id)) {
			throw new NotFoundException(__('Invalid persona'));
                ...
                //echo $id;
		$options = array('conditions' => array('Persona.user_id' => $id));
		...
		$options = array('conditions' => array('Persona.nucleo_familiar_ref' => $id));
 ...
 */
	public function edit($id = null) {
		...
		if ($this->request->is(array('post', 'put'))) {
			...
			$options = array('conditions' => array('Persona.cedula' => $cedula));
			$this->request->data = $this->Persona->find('first', $options);
		}
                $options = array('conditions' => array('Persona.cedula' => $cedula));
			$this->set('persona', $this->Persona->find('first', $options));
		...
		$tipoPersonas = $this->Persona->TipoPersona->find('list', array('conditions'=>array('TipoPersona.id !='=>5,'TipoPersona.id <>'=>3 )));
		$alumnos = $this->Persona->Alumno->find('list');
                $persona = $this->Persona->find('all', array('fields'=>array('foto'),'conditions'=>array('Persona.id'=> $id)));
		//echo debug($persona);
                $this->set(compact('estados','persona', 'municipios', 'parroquias', 'sexos', 'cargos', 'users', 'tipoPersonas', 'alumnos'));
	}
		...
		$sexos = $this->Persona->Sexo->find('list');
		$cargos = $this->Persona->Cargo->find('list');
                $tipo_per = $this->Persona->find('all', array('conditions'=>array('Persona.user_id' => $id)));
                //aqui busco los id de todas las personas que sean madres o padres
                $parentesco = $this->Persona->PersonasTipoPersona->find('list', array('fields'=>array('PersonasTipoPersona.persona_id'),'conditions'=>array('OR'=>array('PersonasTipoPersona.tipo_persona_id'=>array(1,2)))));
                //aqui busco los datos de la persona que sea la madre de este nucleo familiar..
                $nucleo_fam = $this->Persona->find('all', array('conditions'=>array('Persona.nucleo_familiar_ref'=> $id, 'Persona.id'=>$parentesco)));
                //echo debug($nucleo_fam);
                //$nucleo_fam = $this->Persona->find('all', array('conditions'=>array('Persona.nucleo_familiar_ref'=> $id)));
                Configure::write('debug', '0');
                if($nucleo_fam == null)
                {
                    $personas = $this->Persona->find('all', array('conditions'=>array('Persona.user_id' => $id)));
...
///funcion utilizada para registrar y/o registrar trabajadores del preescolar
    ...
    public function asignacion()
		...
		if ($this->request->is(array('post', 'put'))) {
                   ...
                   $id = $this->request->data['GradosSeccione']['GradosSeccione']['0'];
                   $existe = $this->Persona->GradosSeccionesPersona->find('all', array('conditions'=>array('GradosSeccionesPersona.persona_id'=>$id_persona)));
                   if(empty($existe))
                   {
                   if ($this->Persona->saveAll($this->request->data)) {
                            $this->Persona->GradosSeccione->query("Update grados_secciones SET asignada = True where id= $id");
			...
			$this->request->data = $this->Persona->find('list');
                ...
                $gradosSecciones = $this->Persona->GradosSeccione->find('list'/*, array('conditions'=>array('GradosSeccione.asignada' => 0))*/);
		...
		$sexos = $this->Persona->Sexo->find('list');
		$cargos = $this->Persona->Cargo->find('list');
		$users = $this->Persona->User->find('list', array('conditions'=>array('User.username'=>$usuario)));
		$tipoPersonas = $this->Persona->TipoPersona->find('list', array('conditions'=>array('TipoPersona.id <=' =>'2')));
		$alumnos = $this->Persona->Alumno->find('list');
		$this->set(compact('estados', 'municipios', 'parroquias', 'sexos', 'cargos', 'users', 'tipoPersonas', 'alumnos')); 
        }
