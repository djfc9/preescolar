...
//echo $años1."-".$meses1."<br>";
   ...
   echo "</td></tr><tr><td>";
		...
		//echo $this->Form->input('fecha_nacimiento', array('style'=>'width: 150px;'));
                ...
                foreach ($datos['0']['GradosSeccione'] as $dato):
                if($dato['ano_escolar']==$ano_escolar){
                    $grado_y_seccion_actual = $dato['descripcion'];
                    $actual = $dato['id'];
                }
                endforeach;
                echo "<b>Grado y Sección Actual: <font color='blue'>".$grado_y_seccion_actual."</font>";
                echo "</td></tr>";
                echo $this->Form->input('Alumno.actual', array('value'=>$actual, 'type'=>'hidden'));
}
else
{
    echo "Busqueda no Produjo resultados";
    echo "<script>alert('Busqueda no Produjo resultados');"
    . "document.location=('/preescolar/Alumnos_Grados_Secciones/inscribir');</script>";
}
                ?>
</table>